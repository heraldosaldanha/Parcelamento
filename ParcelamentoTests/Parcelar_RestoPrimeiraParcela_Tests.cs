...
namespace Parcelamento.Tests
{
    ...
    public class Parcelar_RestoPrimeiraParcela_Tests
    {
        ...
        [DataRow(false)]
        [DataRow(true)]
        public void RestoPrimeiraParcela_ComResto_Test(bool negativo)
        {
            var modulo = negativo ? -1M : 1M;
            var expected = new SortedDictionary<int, decimal>();
            expected.Add(1, 1.70M * modulo);
            ...
            //Action
            var resultado = Parcelar.RestoPrimeiraParcela(10M * modulo, 6);
        ...
        [DataRow(false)]
        [DataRow(true)]
        public void RestoPrimeiraParcela_SemResto_Test(bool negativo)
        {
            var modulo = negativo ? -1M : 1M;
            var expected = new SortedDictionary<int, decimal>();
